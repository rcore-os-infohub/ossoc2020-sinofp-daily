                    ...
                    if let Some(init_data) = init_data {
                        if !init_data.is_empty() {
                            ...
                            let start = if segment.range.start > page_address {
                                segment.range.start.0 - page_address.0
                            } else {
                                0
                            };
                            let stop = min(PAGE_SIZE, segment.range.end.0 - page_address.0);
                            // 计算来源和目标区间并进行拷贝
                            let dst_slice = &mut page_data[start..stop];
                            let src_slice = &init_data[(page_address.0 + start - segment.range.start.0)
                                ..(page_address.0 + stop - segment.range.start.0)];
